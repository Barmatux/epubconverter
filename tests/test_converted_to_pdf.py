...
import io
from converter.converter_2_pdf import _generate_new_name, _read_stream, convert_to_user_format, convert

class TestConvertedToPdfFunctions(unittest.TestCase):

    def test_generate_new_name_file_from_url(self):
    ...
    @patch('converter.converter_2_pdf._generate_new_name')
    def test_convert(self, mock_generate_new_name, mock_convert):
        original_path = 'origin_path'
        output_format = 'epub'
        url_path = 'temp_path'
        mock_generate_new_name.return_value = 'test.epub'
        assert convert(url_path, original_path, output_format) == 'test.epub'
        mock_generate_new_name.assert_called_once_with(original_path, output_format)
